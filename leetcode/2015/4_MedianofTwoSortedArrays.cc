...
int inline adjust(int num_bound, int i_free, vector<int>& nums_free) {
    int c;
    if(num_bound > nums_free[i_free-1]) {
        if(i_free < nums_free.size() && num_bound < nums_free[i_free]) {
            c = num_bound;
        }
        else {
            c = nums_free[i_free];
        ...
        if(i_free-1 > 0 && num_bound > nums_free[i_free-2]) {
            c = num_bound;
        }
        else {
            c = nums_free[i_free-2];
        }
    }
    return c;
}
double findMedianSortedArrays_fail1(vector<int>& nums1, vector<int>& nums2) {
    int len1 = nums1.size(), len2 = nums2.size(), b1 = 0, b2 = 0, e1 = len1+1, e2 = len2+1;
    int i1 = len1, i2 = len2, median_i = (len1 + len2 + 1) / 2;
    while(b1 < e1-2 && b2 < e2-2 && i1 + i2 -1 != median_i) {
        if(i1 + i2 - 1 > median_i) {
            if(nums1[i1-1] > nums2[i2-1] || nums1[i1-1] == nums2[i2-1] && i1>e1 && i2>e2 && nums1[i1-2] > nums2[i2-2]) {
                e1 = i1;
                i1 = b1 + (e1-b1) / 2;
            }
            else {
    ...
    int c1 = nums1[i1-1], c2 = nums2[i2-1];
    if(i1 == 1 || i1 == len1) {
        c1 = adjust(nums1[i1-1], i2, nums2);
        c2 = nums2[i2 - 1];
    }
    else if(i2 == 1 || i2 == len2) {
        c2 = adjust(nums2[i2-1], i1, nums1);
        c1 = nums1[i1 = 1];
    }
    return (len1 + len2) % 2 ? max(c1, c2) : 1.0*(c1+c2) / 2;
}

double findMedianSortedArrays_new(vector<int>& nums1, vector<int>& nums2) {
    int len1 = nums1.size(), len2 = nums2.size();
    if(len1 > len2)
        return findMedianSortedArrays_new(nums2, nums1);
    int spliter = (len1 + len2) / 2;
    int low_bound = 0, up_bound = len1, i, j;
    ...
    while(low_bound <= up_bound) {
        ...
        if(i > 0 && j < len2 && nums1[i-1] > nums2[j])
            ...
            return low_max + (high_min - low_max) / 2.0;
        }
    }
}
/*
 * This method make improvement based on this: https://leetcode.com/discuss/9265/share-my-simple-o-log-m-n-solution-for-your-reference
 * I think I've improved the complexity from O(log(m+n)) to O(log(min(m,n)))
 * This method is same as the M2 one in python
 */
...
double findMedianSortedArrays_M2(vector<int>& nums1, vector<int>& nums2) {
    int len1 = nums1.size(), len2 = nums2.size();
    int k = (len1 + len2 + 1) >> 1;
    if((len1 + len2) & 1)
...
double findMedianSortedArrays_fail2(vector<int>& nums1, vector<int>& nums2) {
    ...
    if(i1 == 0 || i1 ==nums1.size()) {
        i2 += (s1 + s2) / 2 - (i1 + i2 + 1);
        c2 = nums2[i2];
        if(i2 > 0 && nums1[i1] < nums2[i2-1])
            c1 = nums2[i2-1];
        if(i2 < s2-1 && nums1[i1] > nums2[i2+1])
            c1 = nums2[i2+1];
    }
    else {
        i1 += (s1 + s2) / 2 - (i1 + i2 + 1);
        c1 = nums1[i1];
        if(i1 > 0 && nums2[i2] < nums1[i1-1])
            c2 = nums1[i1-1];
        if(i1 < s1-1 && nums2[i2] > nums1[i1+1])
            c2 = nums1[i1+1];
    }
    return (s1 + s2) % 2 ? max(c1, c2) : 1.0*(c1+c2) / 2;
}
